...
powcor.shrink = function(x, alpha, lambda, w, collapse=FALSE, verbose=TRUE)
{
  if (missing(alpha)) stop("Please specify the exponent alpha!")
   ...
   powr = pvt.powscor(x=x, alpha=alpha, lambda=lambda, w=w, 
...
pvt.powscor = function(x, alpha, lambda, w, collapse, verbose)
{  
  ...
  if (z$lambda == 1 | alpha == 0) # result in both cases is the identity matrix
  {
    if (collapse)
    {
      powr = rep(1, p)  # return only diagonal vector to save memory
      names(powr) = colnames(xs)
    }
    else
    {
      powr = diag(p)    # return complete matrix
